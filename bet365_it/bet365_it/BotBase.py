...
class BotBase(object):
    ...
    def get_db(self):
        db = mysql.connector.connect(user=self.options.username, password=self.options.password,
                                     database=self.options.database, host=self.options.host)
        db.autocommit = False
    ...
    def get_response(self, url, headers, proxy=None, use_residential_proxies=False, post_data=None, post_is_json=False, no_proxy=False, timeout=4, cookies=None, use_tor=False):
        ...
        try:
            if post_data is None:
                response = self.s.get(
                    url, headers=headers, proxies=proxy, cookies=cookies, verify=False,  timeout=timeout)
            else:
                if not post_is_json:
                    response = self.s.post(url, data=post_data, headers=headers,
                                           proxies=proxy, cookies=cookies, verify=False,  timeout=timeout)
                else:
                    response = self.s.post(url, json=post_data, headers=headers,
                                           proxies=proxy, cookies=cookies, verify=False,  timeout=timeout)
    ...
    def get_sql_file(self, file_type, book_id):
        try:
            latest_file = sorted([[os.path.join(i[0], j) for j in i[2] if file_type in j]
                                 for i in os.walk(self.data_directory)][0])[-1]
            if os.stat(latest_file).st_size > 20000:
                latest_file = os.path.join(
                    self.data_directory, file_type+"_"+str(book_id)+"_"+str(int(time()*1000))+".sql")
        except Exception:
            latest_file = os.path.join(
                self.data_directory, file_type+"_"+str(book_id)+"_"+str(int(time()*1000))+".sql")
    ...
    def commit_sql2(self, sql):
        sql_file = self.get_sql_file(
            "save_data", self.bot_data["book_data"]["id"])
    ...
    def save_data(self, data):
        try:
            ...
            if data and "type" in data:
                ...
                events_data = ""
                competitions_data = ""
                update_time = datetime.datetime.now(pytz.timezone(
                ...
                if data["type"] == "odds_data":
                    ...
                    for bet_type, book_odds in data["odds"].items():
                        ...
                        if str(data["bookie_id"]) == '27':
                            if data["event_id"] in self.pinterbet_events:
                                odds_data += 'insert into bookie_odds (event_id,bookie_id,market_id,selection_id,odds,game_play,update_time,cycle) values({0},{1},{2},{3},{4},{5},NOW(),{6}) on duplicate key update odds={4},update_time=NOW(),cycle={6},hidden=0;'.format(
                                    self.pinterbet_events[data["event_id"]], "33", market, selection, book_odds, data["game_play"], self.start_time).replace("'NULL'", "NULL")

                        odds_data += 'insert into bookie_odds (event_id,bookie_id,market_id,selection_id,odds,game_play,update_time,cycle) values({0},{1},{2},{3},{4},{5},NOW(),{6}) on duplicate key update odds={4},update_time=NOW(),cycle={6},hidden=0;'.format(
                            data["event_id"], data["bookie_id"], market, selection, book_odds, data["game_play"], self.start_time).replace("'NULL'", "NULL")
                    ...
                    for event in data["events"]:
                        ...
                        params = '{\\"event_id\\":\\"'+str(
                            event["event_id"])+'\\",\\"params\\":\\"'+str(event["params"])+'\\"}'
                        if str(data["bookie_id"]) == "27":
                            params = '{\\"event_id\\":\\"'+str(
                                event["event_id"])+'|33\\",\\"params\\":\\"'+str(event["params"])+'\\"}'
                            if data["competition_id"] in self.pinterbet_competitions:
                                if betradar_id is None or betradar_id == "NULL":
                                    events_data += 'insert into events (event,open_date,betradar_id,bookie_id,params,competition_id,update_time) values ("{0}","{1}",{2},{3},"{4}",{5},NOW()) on duplicate key update event="{0}",open_date="{1}",update_time=NOW(),params="{4}";'.format(
                                        (event["home"]+" v "+event["away"]), event["open_date"], betradar_id, "33", params, self.pinterbet_competitions[data["competition_id"]]).replace('"NULL"', 'NULL')
                                else:
                                    events_data += 'insert into events (event,open_date,betradar_id,bookie_id,params,competition_id,update_time) values ("{0}","{1}",{2},{3},"{4}",{5},NOW()) on duplicate key update event="{0}",open_date="{1}",betradar_id={2},update_time=NOW(),params="{4}";'.format(
                                        (event["home"]+" v "+event["away"]), event["open_date"], betradar_id, "33", params, self.pinterbet_competitions[data["competition_id"]]).replace('"NULL"', 'NULL')
                        if betradar_id is None or betradar_id == "NULL":
                            events_data += 'insert into events (event,open_date,betradar_id,bookie_id,params,competition_id,update_time) values ("{0}","{1}",{2},{3},"{4}",{5},NOW()) on duplicate key update event="{0}",open_date="{1}",update_time=NOW(),params="{4}";'.format(
                                (event["home"]+" v "+event["away"]), event["open_date"], betradar_id, data["bookie_id"], params, data["competition_id"]).replace('"NULL"', 'NULL')
                        else:
                            events_data += 'insert into events (event,open_date,betradar_id,bookie_id,params,competition_id,update_time) values ("{0}","{1}",{2},{3},"{4}",{5},NOW()) on duplicate key update event="{0}",open_date="{1}",betradar_id={2},update_time=NOW(),params="{4}";'.format(
                                (event["home"]+" v "+event["away"]), event["open_date"], betradar_id, data["bookie_id"], params, data["competition_id"]).replace('"NULL"', 'NULL')
                    ...
                    if "odds" in data["events"][0]:
                        ...
                        for event in data["events"]:
                            ...
                            if "id" in event:
                                ...
                                event_hash = hashlib.md5((event["home"]+" v "+event["away"]+str(
                                    data["competition_id"])+data["bookie_id"]).encode('utf-8')).hexdigest()
                                if event_hash in self.event_keys:
                                    event_id = self.event_keys.get(
                                        event_hash, None)
                            if event_id:
                                dt = {"type": "odds_data", "betradar_id": betradar_id, "event_id": event_id,
                                      "odds": event["odds"], "game_play": game_play, "bookie_id": data["bookie_id"]}
                                self.save_data(dt)
                    ...
                    for competition in data["competitions"]:
                        if not max([c in competition["competition_name"].lower() or c in competition["country_name"].lower() for c in ["speciali", "sanzioni", "minuto 1° goal", "calci d'angolo"]]):
                            competitions_data += 'insert into competitions (competition,sport_id,country,bookie_id,params,update_time) values ("{0}",{1},"{2}",{3},"{4}",NOW()) on duplicate key update update_time=NOW();'.format(
                                (competition["competition_name"]), self.sprt, competition["country_name"], data["bookie_id"], (competition["params"])).replace('"NULL"', 'NULL')
                            if str(data["bookie_id"]) == "27":
                                competitions_data += 'insert into competitions (competition,sport_id,country,bookie_id,params,update_time) values ("{0}",{1},"{2}",{3},"{4}",NOW()) on duplicate key update update_time=NOW();'.format(
                                    (competition["competition_name"]), self.sprt, competition["country_name"], "33", (competition["params"]+"|33")).replace('"NULL"', 'NULL')
                ...
                if str(self.bot_data["book_data"]["id"]) == "2" and competitions_data and "events" in data["competitions"][0]:
                    ...
                    for competition in data["competitions"]:
                        competition_hash = hashlib.md5((competition["competition_name"]+str(self.bot_data["book_data"]["id"])+str(
                            self.sprt)+competition["country_name"]).encode('utf-8')).hexdigest()
                        if len(competition["events"]) and competition_hash in self.competition_keys:
                            ...
                            events_data = {
                                "type": "events_data", "events": competition["events"], "competition_id": competition_id, "bookie_id": self.bot_data["book_data"]["id"]}
            ...
            try:
                ...
                if self.sprt != Sport.CALCIO.value and (time() - self.start_time2) >= 60 or self.sprt == Sport.CALCIO.value and (time() - self.start_time2) >= 600:
                    ...
                    if(len(all_competitions) > 0):
                        data = {"type": "competitions_data", "competitions": all_competitions,
                            ...
                            try:
                                if not max([c in competition["competition_name"].lower() or c in competition["country_name"].lower() for c in ["speciali", "sanzioni", "minuto 1° goal", "calci d'angolo"]]):
                                    if "events" in competition and "competition_id" in competition:
                                        ...
                                        bookie_id = self.bot_data["book_data"]["id"]
                                        events_data = {
                                            "type": "events_data", "events": competition["events"], "competition_id": competition["competition_id"], "bookie_id": bookie_id}
                                        self.save_data(events_data)
                                        got_events = True
                            except Exception:
                                exc_type, exc_obj, exc_tb = sys.exc_info()
        ...
        try:
            if "params" in competition and competition["params"] != None and competition["params"] != "NULL":
                ...
                if events and len(events) > 0:
                    bookie_id = self.bot_data["book_data"]["id"]
                    data = {"type": "events_data", "events": events,
                            "competition_id": competition["competition_id"], "bookie_id": bookie_id}
                    self.save_data(data)
    ...
    def process_events(self, event):
        try:
            ...
            if all_book_odds:
                bookie_id = self.bot_data["book_data"]["id"]
                data = {"type": "odds_data",
                        "event_id": event["id"], "odds": all_book_odds, "game_play": game_play, "bookie_id": bookie_id}
                self.save_data(data)
        except Exception:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            traceback.print_exception(exc_type, exc_obj, exc_tb)
    ...
    def get_matched_bookie_events(self):
        ...
        db = self.get_db()
        try:
            sql = "select e.id,e.params,md5(CONCAT(e.event,e.competition_id,e.bookie_id)) from events as e inner join competitions as c on c.id=e.competition_id where e.bookie_id={0} and c.sport_id={1} and e.open_date>NOW()".format(
                self.bot_data["book_data"]["id"], self.sprt)
            cur = db.cursor()
            cur.execute(sql)
            rows = cur.fetchall()
            cur.close()
    ...
    def get_saved_competitions(self):
        ...
        db = self.get_db()
        try:
            sql = "select id,competition,bookie_id,sport_id,country,params from competitions where bookie_id={0} and sport_id={1} and TIMESTAMPDIFF(HOUR,update_time,NOW()) < 4 ".format(
                self.bot_data["book_data"]["id"], self.sprt)
            cur = db.cursor()
            cur.execute(sql)
            rows = cur.fetchall()
            cur.close()
            if len(rows) == 0:
                sql = "select id,competition,bookie_id,sport_id,country,params from competitions where bookie_id={0} and sport_id={1} ".format(
                    self.bot_data["book_data"]["id"], self.sprt)
                cur = db.cursor()
                cur.execute(sql)
                rows = cur.fetchall()
            ...
            for row in rows:
                if row[4] != None and row[4] != "NULL":
                    all_competitions.append(
                        {"params": row[5], "country_name": row[4], "competition_name": row[1], "competition_id": row[0]})
                competition_hash = hashlib.md5(
                    (row[1]+str(row[2])+str(row[3])+str(row[4]).replace("None", "NULL")).encode('utf-8')).hexdigest()
    ...
    def init_competition_keys(self):
        ...
        try:
            ...
            sql = "select id,competition,bookie_id,sport_id,country from competitions where bookie_id={0} and sport_id={1}".format(
                self.bot_data["book_data"]["id"], self.sprt)
            cur = db.cursor()
            cur.execute(sql)
            rows = cur.fetchall()
    ...
    def init_event_keys(self):
        ...
        try:
            ...
            sql = "select e.id,e.event,e.competition_id,e.bookie_id from events as e where e.bookie_id={0}".format(
                self.bot_data["book_data"]["id"])
            cur = db.cursor()
            cur.execute(sql)
            rows = cur.fetchall()
            cur.close()
            ...
            for row in rows:
                event_hash = hashlib.md5(
                    (row[1]+str(row[2])+str(row[3])).encode('utf-8')).hexdigest()
                    ...
                    if self.threads == 1:
                        ...
                        try:
                            with closing(pathos.multiprocessing.Pool(processes=self.threads)) as p:
                                results = p.map_async(
                                    self.process_leagues_v2, all_leagues).get(timeout=1200)
                                p.terminate()
                    ...
                    if all_events:
                        ...
                        if self.threads == 1:
                            ...
                            try:
                                with closing(pathos.multiprocessing.Pool(processes=self.threads)) as p:
                                    results = p.map_async(
                                        self.process_events, all_events).get(timeout=1200)
                                    p.terminate()
                            except KeyboardInterrupt:
                                p.terminate()
                                p.join()
